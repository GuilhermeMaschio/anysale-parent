...
public interface LeadTaskJpaRepository extends JpaRepository<LeadTask, UUID> {
 ...
 @Modifying @Query("update LeadTask t set t.status='OPEN', t.assignedTo=null, t.reservedAt=null, t.reservationExpiresAt=null where t.tenantId=:tenant and t.status='ASSIGNED' and t.reservationExpiresAt < :now") int releaseExpired(@Param("tenant") String tenant,@Param("now") Instant now);
